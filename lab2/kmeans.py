...
def main():
	...
	for n in n_clusters:
		...
		print str(n)+" clusters being calculated..."

		start = time.clock()
		if distance_func == 'ed':
			clusterer = nltk.cluster.KMeansClusterer(n, nltk.cluster.euclidean_distance, repeats=3, initial_means=None, avoid_empty_clusters=True)
		else:
			clusterer = nltk.cluster.KMeansClusterer(n, nltk.cluster.cosine_distance, repeats=3, initial_means=None, avoid_empty_clusters=True)
		cluster_labels = clusterer.cluster(data, True)
		end = time.clock()
